...
const root = process.cwd(), source = path.join(root, "extension"), output = path.join(source, "dist");
...
await build({ configFile: false, root, publicDir: false, build: { emptyOutDir: false, minify: "esbuild", target: "chrome120", lib: { entry: path.join(source, "src/content.js"), formats: ["iife"], name: "PromptNestContent", fileName: () => "content.js" }, outDir: output } });

for (const file of ["manifest.json", "popup.html", "popup.css", "privacy.html", "content.css"]) {
...
if (manifest.manifest_version !== 3) throw new Error("Manifest V3 requis");
await writeFile(path.join(output, "BUILD_INFO.txt"), `PromptNest ${manifest.version}\nExtension ID: beflijeobbbggeimbknjohpckbkdaphb\nOAuth redirect: https://beflijeobbbggeimbknjohpckbkdaphb.chromiumapp.org/supabase\n`);
